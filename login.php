...
$uname=$_POST['un'];
$pwd=$_POST['pw'];
$query="select * from user where uname='$uname' and password='$pwd'";
$b=mysql_query($query);
if(!$b)
{
	echo'error occured';
...
else
{
	...
	if(!empty($line))
	{
		...
		$_SESSION["pwd"]="$pwd";
?>
	<form name ="login" action="result.php" method="post" onsubmit="return validateForm()">
		<?php
      $connection = mysqli_connect("localhost","root","saksham","room") or die("Error " . mysqli_error($connection));
      $sql = "select name from rooms";
      $result = mysqli_query($connection, $sql) or die("Error " . mysqli_error($connection));
    ...
    <?php mysqli_close($connection); ?>
	<p><b><br>Start time:</b> Hour: <input type="number" name="hi" min="0" max="23" /> Min: <input type="number" name="mi" min="0" max="59"> *24 hr format</p>
	<p><b> End time :</b> Hour: <input type="number" name="he" min="0" max="23" /> Min: <input type="number" name="me" min="0" max="59"> *24 hr format</p>
...
<?php		
	...
	{
?>
<script type="text/javascript">alert("Invalid userid/password");window.location.href='1.php';
    </script>
    <?php
    }
    }
